...
function merge (add_on_lists) {
    ...
    add_on_lists.forEach(a => {
        installs = installs.concat(a.installs);
        manual_installs = manual_installs.concat(a.manual_installs);
    });
...
exports.Fetcher = function(){
    ...
    return {
        ...
        start: () => {
            ...
            let deferred = defer();
            let master_reject = () => deferred.reject();
            let new_destroyers = [];
            ...
            promise.then(r => {
                ...
                all(page_promises).then(add_on_lists => {
                    add_on_lists.unshift(r);
                    deferred.resolve(merge(add_on_lists));
                }, master_reject).then(noop, console.log);
            }, master_reject);
...
// fetch(target)[0].then(r => console.log(r));
// fetch(target, true)[0].then(r => console.log(r));
