...
function fetch(url, fetch_total) {
    ...
    // a destroy function used to terminate the fetching. The
    // destroy function rejects the promise.
    ...
    worker.port.on("no-add-ons-found", info => {
        console.log("could not find addon: ", info);
    ...
    function destroyer () {
        ...
        find_add_ons.reject(Error("aborted"));
        find_total.reject(Error("aborted"));
    ...
    if (fetch_total){
        master = all([find_add_ons.promise, find_total.promise])
            .then(([add_on_list, total_addons]) => {
                return FetchResult(add_on_list, total_addons);
            });
    }
    const destory_worker = () => {
        if (!destroyed) {
            worker.destroy();
        }
    };
    master.then(destory_worker, destory_worker);
...
function merge (add_on_lists) {
    ...
    return {installs, manual_installs};
}
