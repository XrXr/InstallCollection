...
function show_sidebar(collection_title) {
    ...
    // when the worker is already there
    if (sidebar_worker && collection_title){
        sidebar_worker.port.emit('collection-title', collection_title);
    }
    function attach_listener (worker) {
    ...
    // don't want to pile up the listeners
    sidebar.removeListener('attach', attach_listener);
    sidebar.once('attach', attach_listener);
    sidebar.show();
}
...
function start_install() {
    if (install_list !== null){  // already in progress
        show_sidebar();
        return;
    }
