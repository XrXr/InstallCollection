...
// if this changes, it is probably not the only change. No need to fetch this dynamically
...
function notify_side_bar (add_ons) {
    sidebar_worker.port.emit("add_ons", add_ons);
}

function start_install_process() {
    show_sidebar();
    all(fetcher.start()).then(notify_side_bar, noop);
}
