...
function update(tab){
    ...
    if (is_collection.test(tab.url)){
        ...
        page_worker.port.on('found-add-ons', function(add_ons){
            show_sidebar(add_ons);
        });
    }
    //we should be destroying the worker when the test fails, but that would cause
    //model.cotentWorker to be null untill the tab is remade, causing an error everytime
    ...
    sidebar.once('attach', function (worker) {
        ...
        worker.port.on("confirm-install",function(add_ons){
            add_ons.installs.forEach(a => AddonManager.
                getInstallForURL(a.link, install_callback(a.name),
...
function install_callback (name) {
    //WoooOOoOo closure
